export type StrategyDirection = '纯多头' | '纯空头' | '单向' | '双向';
export type StrategyMethod = '趋势跟踪' | '均值回归' | '剥头皮' | '马丁' | '网格' | '突破' | '事件驱动' | '波动率' | '套利过滤';
export type StrategyKind = '纯技术' | '事件型' | '重大数据型';
export type StrategySession = '亚盘策略' | '欧盘策略' | '美盘策略' | '全天候';
export type StrategyHorizon = '日内高频' | '日内短线' | '中期波段' | '长期趋势';
export type StrategyStyle = '稳健' | '中风险' | '激进' | '暴利' | '防守';
...
export type StrategyRuntimeStatus = '模拟运行' | '观察中' | '暂停';

export interface GoldStrategyRuntime {
...
export const GOLD_STRATEGY_LIBRARY: GoldStrategy[] = [
...
const STYLE_BUDGET: Record<StrategyStyle, number> = {
  稳健: 42,
  中风险: 26,
  激进: 17,
  暴利: 5,
...
export function getStrategyCapitalPct(strategy: GoldStrategy): number {
...
export function getStrategyDecision(strategy: GoldStrategy): '运行' | '观察' | '暂停' {
...
function getTradeFrequency(strategy: GoldStrategy): number {
  if (strategy.horizon === '日内高频') return 18;
  if (strategy.horizon === '日内短线') return 9;
  if (strategy.horizon === '中期波段') return 4;
  return 2;
}

function getStyleVolatility(style: StrategyStyle): number {
  if (style === '防守') return 0.55;
  if (style === '稳健') return 0.85;
  if (style === '中风险') return 1.15;
  if (style === '激进') return 1.55;
  return 2.2;
}
...
export function getStrategyRuntime(
  ...
  const allocatedCapital = Number(((totalCapital * getStrategyCapitalPct(strategy)) / 100).toFixed(2));
  const dayKey = getDayKey(date);
  const directionSeed = seededRatio(`${strategy.id}:${dayKey}:pnl`);
  const winSeed = seededRatio(`${strategy.id}:${dayKey}:win`);
  const tradeSeed = seededRatio(`${strategy.id}:${dayKey}:trades`);
  const positionSeed = seededRatio(`${strategy.id}:${dayKey}:position`);
  const volatility = getStyleVolatility(strategy.style);
  const riskFactor = Math.max(0.35, strategy.riskScore / 100);
  const activeScale = decision === '运行' ? 1 : decision === '观察' ? 0.28 : 0;
  const rawPct = (directionSeed - 0.46) * volatility * riskFactor * 3.8 * activeScale;
  const pnlPct = Number(rawPct.toFixed(2));
  const pnl = Number(((allocatedCapital * pnlPct) / 100).toFixed(2));
  const trades = decision === '暂停'
    ? 0
    : Math.max(1, Math.round(getTradeFrequency(strategy) * activeScale * (0.65 + tradeSeed)));
  const winRate = decision === '暂停'
    ? 0
    : Number(Math.min(82, Math.max(34, 50 + (winSeed - 0.45) * 28 - strategy.riskScore * 0.04)).toFixed(1));
  const maxDrawdown = decision === '暂停'
    ? 0
    : Number(Math.max(0.2, Math.abs(rawPct) * 0.72 + volatility * 0.55 + strategy.riskScore / 90).toFixed(2));

  return {
    status: decision === '运行' ? '模拟运行' : decision === '观察' ? '观察中' : '暂停',
    allocatedCapital,
    equity: Number((allocatedCapital + pnl).toFixed(2)),
    pnl,
    pnlPct,
    winRate,
    maxDrawdown,
    trades,
    openPosition: decision === '运行' && positionSeed > 0.42,
