...
| **事件数据接入** | `docs/05-事件数据接入.md` | 经济事件和快讯数据接入说明 | ⭐⭐ |
...
| 文件 | 路径 | 说明 |
|------|------|------|
| 爬虫服务 | `goldpilot-backend/src/services/scraper.ts` | 获取经济日历和快讯 |
| 事件控制器 | `goldpilot-backend/src/controllers/events.ts` | 事件API处理 |
| API路由 | `goldpilot-backend/src/routes/api.ts` | 所有API路由 |
| 定时任务 | `goldpilot-backend/src/services/scheduler.ts` | 定时刷新数据 |

### 前端核心文件
| 文件 | 路径 | 说明 |
|------|------|------|
| 数据服务 | `goldpilot-frontend/src/services/data.ts` | 事件数据API调用 |
| 首页组件 | `goldpilot-frontend/src/pages/Home/Home.tsx` | 主页面，展示所有数据 |
| 事件类型 | `goldpilot-frontend/src/types/event.ts` | Event和Flash类型定义 |
